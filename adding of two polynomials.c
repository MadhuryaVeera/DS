//Adding poolynomoial reoresentation using linked list//
#include<stdio.h>
#include<stdlib.h>
struct node{
	float coeff;
...
typedef struct node NODE;//fun definition//
NODE* insert_a_term(NODE *head,float co,int ex){
	NODE*new_term=(NODE*)malloc(sizeof(NODE));
	new_term->coeff=co;
	new_term->expo=ex;
	new_term->next=NULL;
	if(head==NULL||ex>head->expo)//new_term->expo>head->expo//
	{
		new_term->next=head;
		head=new_term;
	}
...
NODE* create_polynomial(){
	NODE *head=NULL;
	int n;
	printf("Enter no. of tems: ");
	scanf("%d",&n);
	int i;
	for(i=1;i<=n;i++){//we have to insert ecach term//
		float co;//local variable
		int ex;
...
NODE* poly_addition(NODE *poly1,NODE *poly2){
	NODE *t1=poly1;
	NODE *t2=poly2;
	NODE *head=NULL;//for resultant polynomoial//
